...
int altaBicicleta(eBicicleta nuevaBicicleta[], int tam, int* idBicicleta)
{
    ...
    if(nuevaBicicleta != NULL && tam>0 && idBicicleta!= NULL)
    {
        ...
        else
        {
            ...
            pedirEntero(&auxAlta.rodado, "Ingrese rodado 20/26/27.5/29: ", "Error. ");
...
int listarBicicletas(eBicicleta lista[], int tam, eTipo tipos[], int tamT, eColor colores[], int tamC)
{
    ...
    for(int i=0; i<tam; i++)
    {
        if(!lista[i].isEmpty)
        {
            mostrarBicicleta(lista[i], tam, tipos, tamT, colores, tamC);
            flag = 0;
        }
    }
    if(flag)
...
int modificarBicicleta(eBicicleta lista[], int tamB, eTipo tipos[], int tamT, eColor colores[], int tamC)
{
    ...
    listarBicicletas(lista, tamB, tipos, tamT, colores, tamC);
    ...
    {
        printf("id\t    Marca          Tipo        Color    Rodado\n\n");
        mostrarBicicleta(lista[indice], tamB, tipos, tamT, colores, tamC);
        switch(subMenuModificar())
        {
            case 1:
                ...
                break;
            case 2:
                pedirEntero(&lista[indice].rodado, "Ingrese nuevo rodado: ", "Error. ");
                break;
            default:
